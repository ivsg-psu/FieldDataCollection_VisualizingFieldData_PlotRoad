function fcn_plotRoad_plotHeadTailLL(LLdata, fig_num, varargin)
%fcn_plotRoad_plotHeadTailLL    plots XY data with user-defined formatting strings
% 
% FORMAT:
%
...
%      corresponds to the [Latitude Longitude] coordinate of the point to
%      plot in the 1st and 2nd column.
%      
...
if flag_do_plots
    ...
    fcn_plotRoad_plotLL(LLdata(1,:), (tempPlotFormat), (fig_num));
    ...
    tempPlotFormat.LineStyle = 'none';
    fcn_plotRoad_plotLL(LLdata(end,:), (tempPlotFormat), (fig_num));

    if length(LLdata(:,1))>4
        ...
        fcn_plotRoad_plotLL(LLdata(1:2,:), (tempPlotFormat), (fig_num));
        ...
        tempPlotFormat.Marker = 'none';
        tempPlotFormat.LineStyle = '-';
        fcn_plotRoad_plotLL(LLdata(end-1:end,:), (tempPlotFormat), (fig_num));

    else

        % Plot green headers - calculated from vector direction
        vector_direction_start = LLdata(2,1:2) - LLdata(1,1:2);
        ...
        offset_start = LLdata(1,1:2) + max_length*unit_vector_direction_start;
        LLA_vector = [LLdata(1,1:2); offset_start];
        ...
        tempPlotFormat.Marker = 'none';
        tempPlotFormat.LineStyle = '-';
        fcn_plotRoad_plotLL(LLA_vector, (tempPlotFormat), (fig_num));

        % Plot red tailers - calculated from vector direction
        vector_direction_end = (LLdata(end,1:2) - LLdata(end-1,1:2));
        ...
        offset_end = LLdata(end,1:2) - max_length*unit_vector_direction_end;
        LLA_vector = [offset_end; LLdata(end,1:2)];
