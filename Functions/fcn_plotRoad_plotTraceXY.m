...
% FORMAT:
%
%      h_plot = fcn_plotRoad_plotTraceXY(XYdata, (plotFormat), (flag_plot_headers_and_tailers), (figNum))
%
% INPUTS:  
%
...
if flag_do_plots == 1
    ...
    if iscell(XYdata)
        ...
        for ith_data = 1:length(XYdata)
            ...
            h_plot(ith_data) = fcn_INTERNAL_plotData(XYdata_to_plot, plotFormat, flag_plot_headers_and_tailers, figNum);
        end
    else
        h_plot = fcn_INTERNAL_plotData(XYdata, plotFormat, flag_plot_headers_and_tailers, figNum);
    end
...
function h_plot = fcn_INTERNAL_plotData(XYdata_to_plot, plotFormat, flag_plot_headers_and_tailers, figNum)

if ~isempty(XYdata_to_plot)
    h_plot = fcn_plotRoad_plotXY(XYdata_to_plot, (plotFormat), (figNum));

    % Plot headers and tailers?
    if flag_plot_headers_and_tailers
        fcn_plotRoad_plotHeadTailXY(XYdata_to_plot, figNum, plotFormat);
    end
end
